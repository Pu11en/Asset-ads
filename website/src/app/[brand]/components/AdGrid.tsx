...
export function AdGrid({ ads, brand, brandColor, approvalState }: Props) {
  ...
  const handleAction = async (adId: string, action: 'approve' | 'bad' | 'reset') => {
    setLoading(adId);
    try {
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-6 p-4 rounded-2xl bg-white/5 border border-white/10">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-white/70">{counts.bad} bad</span>
        </div>
        <div className="ml-auto flex items-center gap-2">
          {counts.pending === 0 ? (
            ...
            <span className="text-white/30 text-sm">
              {counts.approved} approved · {counts.bad} bad · {counts.pending} pending
            </span>
          )}
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="relative aspect-square bg-black/60">
                ...
                {isBad && (
                  <div className="absolute top-3 left-3 px-3 py-1.5 rounded-full bg-red-500/80 text-white text-xs font-medium">
                    Bad
                  </div>
                )}
                {isPending && (
              ...
              <div className="p-4">
                ...
                {isPending && (
                  <div className="flex gap-2">
                    ...
                    <button
                      disabled={isLoading}
                      onClick={() => handleAction(ad.id, 'bad')}
